...
An important requirement of CDS is that it provide clinically relevant information to the clinician at the right time. This implementation guide assumes that CDS is provided as a real-time remote service which the EHR (client) subscribes to at various points in the workflow of clinicians. The service requests clinical decision support based on the current patient context. The EHR client sends relevant contextual information as part of the request and receives clinically relevant suggestions describing potential actions to be taken. The CDS service should run efficiently to meet clinician expectations and not interrupt from the clinical workflow. Delays in presenting CDS information may lead to unsuccessful CDS adoption and clinician frustration. In this implementation guide, a remote CDS service is not a component of the EHR and uses APIs and web standards for communication. It could be a servlet container hosted within the organization's data architecture / firewall or hosted offsite, and in either case HIPAA privacy and data protection rules apply to all aspects of the interaction between the EHR and the CDS service.
...
A third option that this implementation guide discusses is CDS at the time a clinician opens a patient record ("patient-view"). Although this event happens outside of computerized provider order entry, we included it as a scenario because there are potentially good reasons to do PDDI CDS at `patient-view`. The `patient-view` hook has broad adoption and may be an appropriate point in the clinical workflow for shared decision-making, including review of current potential interactions and related risk calculations.
...
The [Use Case](./use-cases.html) section of this implementation guide describes a representative clinical PDDI example that is used for illustration throughout this implementation guide. PDDI CDS artifacts were created from the use case at a narrative, semi-structured, and fully executable level. The artifacts comply with the minimal information model for potential drug-drug interaction knowledge and evidence described in a W3C Community Interest Group Note.<sup>[8,9](#references)</sup> The IG use cases (Warfarin + non-steroidal anti-inflamatory drugs (NSAIDs) and Digoxin + Cyclosporine) show that the potential interactions often need to consider route and formulation. The minimal information model specifies contextual Information/modifying factors which these would fall under.
...
In addition, these artifacts have been tested over the years in various ways including at connectathons and, most recently, using the VS code environment: [pddi-cds-vscode-validation](https://github.com/dbmi-pitt/pddi-cds-vscode-validation)
